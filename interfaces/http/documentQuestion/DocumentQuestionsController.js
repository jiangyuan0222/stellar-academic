...
const DocumentQuestionsController = {
  get router() {
    const router = Router();
  ...
  addDocumentQuestion(req, res, next) {
    ...
    try {
      ...
      const { SUCCESS, ERROR, VALIDATION_ERROR } = addDocumentQuestion.outputs;
      ...
      logger.error(`Add document question Error : ${error}`);
  ...
  getDocumentQuestion(req, res, next) {
    ...
    try {
      ...
      logger.error(`Get document question Error : ${error}`);
      switch (error.message) {
        case 'UnAuthenticated':
          res
            .status(Status.UNAUTHORIZED)
...
module.exports = DocumentQuestionsController;
